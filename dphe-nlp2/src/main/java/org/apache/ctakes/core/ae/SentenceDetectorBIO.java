 ...
 *
 * @see ListSentenceFixer
 * @see ParagraphSentenceFixer
...
public class SentenceDetectorBIO extends CleartkAnnotator<String>{

  private Logger logger = Logger.getLogger( SentenceDetectorBIO.class);
  ...
  public void process(JCas jcas) throws AnalysisEngineProcessException {
    ...
    for ( Annotation seg : getSegments( jcas ) ){
  ...
  static private Collection<Annotation> getSegments( final JCas jCas ) {
    final Collection<Annotation> segments = new ArrayList<>();
    final Collection<Paragraph> paragraphs = JCasUtil.select( jCas, Paragraph.class );
    if ( paragraphs != null && !paragraphs.isEmpty() ) {
      segments.addAll( paragraphs );
    } else {
      segments.addAll( JCasUtil.select( jCas, Segment.class ) );
    }
    return segments;
  }

  static private final Pattern NEWLINES = Pattern.compile( "\n+" );
  ...
  private void buildDocEndlineModel(JCas jcas) {
    ...
    maxLineLength = -1;
    for ( Annotation seg : getSegments( jcas ) ){
      String[] lines = NEWLINES.split( seg.getCoveredText() );
  ...
  public static AnalysisEngineDescription getDataWriter(File outputDirectory, Class<? extends DataWriter<?>> class1)
        throws ResourceInitializationException {
    return AnalysisEngineFactory.createEngineDescription(
        SentenceDetectorBIO.class,
        SentenceDetectorBIO.PARAM_IS_TRAINING,
        true,
        DirectoryDataWriterFactory.PARAM_OUTPUT_DIRECTORY,
        outputDirectory,
        DefaultDataWriterFactory.PARAM_DATA_WRITER_CLASS_NAME,
        class1,
        SentenceDetectorBIO.PARAM_FEAT_CONFIG,
        FEAT_CONFIG.CHAR);
  }
