...
//  Why?
   ...
   static private final Logger LOGGER = Logger.getLogger( "ListsTableFileWriter" );
...
//   abstract protected List<List<String>> createDataRows( JCas jCas );
   protected List<List<String>> createDataRows( final JCas jCas ) {
      //
      return JCasUtil.select( jCas, ListEntry.class )
                     .stream()
                     .sorted( Comparator.comparingInt( Annotation::getBegin ) )
                     .map( ListEntryRow::new )
                     .map( ListEntryRow::getColumns )
...
//      return Collections.emptyList();
   ...
   static private class ListEntryRow {
      ...
      private final String _entryText;
      private final String _name;
      private final String _value;

      private ListEntryRow( final ListEntry listEntry ) {
         _entryText = listEntry.getCoveredText();
         String name = "";
         String value = "";
         final String[] colonSplits = StringUtil.fastSplit( _entryText, ':' );
         if ( colonSplits.length > 1 ) {
            name = colonSplits[ 0 ].trim();
            for ( int i = 1; i < colonSplits.length; i++ ) {
               value = value + " " + colonSplits[ i ].trim();
            }
         } else {
            final String[] spaceSplits = _entryText.trim()
                                                   .split( "[ ]{2,12}|[\\t]{1,3}" );
            if ( spaceSplits.length > 1 ) {
               name = spaceSplits[ 0 ].trim();
               for ( int i = 1; i < spaceSplits.length; i++ ) {
                  value = value + " " + spaceSplits[ i ].trim();
               }
            }
         }
         _name = name;
