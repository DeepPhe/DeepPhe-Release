 ...
 */
@PipeBitInfo(
        name = "XMI Writer Lines ",
        description = "Writes XMI files with full representation of input text and all extracted information, using separate lines for different elements.",
        role = PipeBitInfo.Role.WRITER,
        dependencies = { PipeBitInfo.TypeProduct.DOCUMENT_ID }
)
public class XmiWriterLines extends CasConsumer_ImplBase {
    /**
    ...
    private void writeXmi( CAS aCas, File name, String modelFileName ) throws IOException, SAXException {
        ...
        try {
            ...
            out = new FileOutputStream( name );
            XmiCasSerializer ser = new XmiCasSerializer( aCas.getTypeSystem() );
            XMLSerializer xmlSer = new XMLSerializer( out, true );  // TODO add parameter to XmiWriterCasConsumerCtakes instead of hardcoding true here for isFormattedOutput, then delete this class. isFormattedOutput=true makes output pretty/prettier
            ser.serialize( aCas, xmlSer.getContentHandler() );
        } finally {
            if ( out != null ) {
                out.close();
    ...
    public static CasConsumerDescription getDescription() throws InvalidXMLException {
        InputStream descStream = XmiWriterLines.class
                .getResourceAsStream( XmiWriterLines.class.getSimpleName() + ".xml" );
        return UIMAFramework.getXMLParser().parseCasConsumerDescription(
                new XMLInputSource( descStream, null ) );
    }

    public static URL getDescriptorURL() {
        return XmiWriterLines.class.getResource( XmiWriterLines.class.getSimpleName() + ".xml" );
    }
}
