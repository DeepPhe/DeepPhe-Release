...
final public class StatusFinder {
   ...
   static private final String TYPE_REGEX
         = "Triple|(?:Estrogen(?:\\s+and\\s+Progesterone)?)|(?:ER(?:sq)?(:(\\s*DCIS)?(\\s*IS)?)?(\\s*BY IHC)?(?:\\s*(?:/|and|or)\\s*(?:PR|(?:HER-? ?2(\\s*/?\\s*neu)?)))?)|(?:Progesterone|PR(?:sq)?(:(\\s*DCIS)?(\\s*IS)?)?(\\s*BY IHC)?)|(?:HER-? ?2(\\s*/?\\s*neu)?(?:\\s*ONCOGENE)?(\\s*BY IHC)?(?:\\s*\\(?ERBB2\\)?)?)";
   static private final String INTERIM_EX
         = "(?:(?:-?\\s*?Receptors?\\s*-?)?\\s*(?:(?:status|expression|test|(?:\\([A-Z 0-9]+\\)))\\s*)?(?:(?:is|are|was)\\s*)?\\s*:?)|(?:\\s+PROTEIN\\s+EXPRESSION\\s+\\(0-3\\+\\):)";
   static private final String STRENGTH_EX = "(?:strong|weak|[0-9%]*)(?:ly)?";
   ...
   private void testMultiples( final String sentence ) {
      ...
      for ( SimpleStatus status : statuses ) {
         System.out.println( sentence.substring( status._begin, status._end ));
      }
   }
   ...
   @Test
   public void testHer2ExamplesFromTrain() { testReceptorStatus( her2ExamplesFromTrain, StatusType.HER2, StatusValue.NEGATIVE );
   }

   private String [] prExamplesFromTrain = new String [] {
   };
   @Test
   public void testPrExamplesFromTrain() { testReceptorStatus( prExamplesFromTrain, StatusType.PR, StatusValue.NEGATIVE );
   }
