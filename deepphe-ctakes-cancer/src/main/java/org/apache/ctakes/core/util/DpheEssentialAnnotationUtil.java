 ...
 * @since 12/29/2017
 */
final public class DpheEssentialAnnotationUtil {

   static private final Logger LOGGER = Logger.getLogger( "DpheEssentialAnnotationUtil" );

   private DpheEssentialAnnotationUtil() {
   }
    ...
    */
   static public Map<IdentifiedAnnotation, Collection<Integer>> createMarkableCorefsOld( final JCas jCas ) {
      final Collection<CollectionTextRelation> corefs = JCasUtil.select( jCas, CollectionTextRelation.class );
      final Map<Markable, IdentifiedAnnotation> markableAnnotations = mapMarkableAnnotationsOld( jCas, corefs );
      return createMarkableCorefsOld2( corefs, markableAnnotations );
   }
   ...
   static public Map<IdentifiedAnnotation, Collection<Integer>> createMarkableCorefs( final JCas jCas ) {
      final Collection<CollectionTextRelation> corefs = JCasUtil.select( jCas, CollectionTextRelation.class );
      final Map<Markable, Collection<IdentifiedAnnotation>> markableAnnotations = mapMarkableAnnotations( jCas, corefs );
    ...
    * @return a map of markables to indexed chain numbers
    */
   static public Map<IdentifiedAnnotation, Collection<Integer>> createMarkableCorefsOld2(
         final Collection<CollectionTextRelation> corefs,
         final Map<Markable, IdentifiedAnnotation> markableAnnotations ) {
      if ( corefs == null || corefs.isEmpty() ) {
         return Collections.emptyMap();
    ...
    */
   static private Map<Markable, IdentifiedAnnotation> mapMarkableAnnotationsOld(
         ...
         for ( Markable markable : markables ) {
            ...
            for ( IdentifiedAnnotation annotation : essentialAnnotations ) {
               ...
               if ( annotation.getEnd() - annotation.getBegin() < bestLength ) {
                  // How did this ever work?  It doesn't require that the markable and annotation overlap
    ...
    * @return map of markable to identified annotation
    */
   static public Map<Markable, Collection<IdentifiedAnnotation>> mapMarkableAnnotations(
         final JCas jCas, final Collection<CollectionTextRelation> corefs ) {
      if ( corefs == null || corefs.isEmpty() ) {
         return Collections.emptyMap();
      }
