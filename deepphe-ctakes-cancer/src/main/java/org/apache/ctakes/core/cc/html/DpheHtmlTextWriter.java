...
final public class DpheHtmlTextWriter extends AbstractJCasFileWriter {
   ...
   static private final Logger LOGGER = Logger.getLogger( "DpheHtmlTextWriter" );
   ...
   public void writeFile( final JCas jCas,
      ...
      try ( final BufferedWriter writer = new BufferedWriter( new FileWriter( htmlFile ) ) ) {
         ...
         // TODO use EssentialAnnotationUtil to create coref chains, map from annotation to text span
   ...
   static private void writeSections( final Map<Segment, Collection<Sentence>> sectionSentences,
      ...
      for ( Segment section : sections ) {
         ...
         for ( Sentence sentence : sentences ) {
            if ( paragraphBegins.contains( sentence.getBegin() ) ) {
               writer.write( "\n</p>\n<p>\n" );
            }
   ...
   static private void writeSections( final Collection<Segment> sectionSet,
      ...
      for ( Segment section : sections ) {
         ...
         sentences.sort( Comparator.comparingInt( Sentence::getBegin ) );
         int currentListEnd = -1;
         boolean inList = false;
         boolean emptyParagraph = true;
         for ( Sentence sentence : sentences ) {
            if ( paragraphBegins.contains( sentence.getBegin() ) && !emptyParagraph ) {
               writer.write( "\n</p>\n<p>\n" );
            }
            ...
            if ( inList ) {
               if ( currentListEnd > 0 && sentence.getBegin() >= currentListEnd ) {
                  endList( writer );
                  inList = false;
                  currentListEnd = -1;
               } else if ( end == null ) {
               ...
               if ( sentence.getEnd() == currentListEnd ) {
                  endList( writer );
                  inList = false;
                  currentListEnd = -1;
                  continue;
   ...
   static private String createPolaritiesText( final Collection<IdentifiedAnnotation> annotations ) {
      ...
      return annotations.stream()
                        .map( DpheHtmlTextWriter::createPolarity )
                        .distinct()
                        .sorted()
   ...
   static private void writeCorefInfos( final Collection<CollectionTextRelation> corefRelations,
      ...
      for ( CollectionTextRelation corefRelation : corefRelations ) {
         ...
         final String text = markables.stream()
                                      ...
                                      .map( DpheHtmlTextWriter::getSafeText )
   ...
   static private String getLegend() {
      return "<div class=\"legend\"><h3>Legend</h3>\n" +
             ...
             "  <table>\n" +
             "    <tr>\n" +
             "      <td>Sign / Symptom<span class=\"" + SemanticMarkup.FINDING_MARK.getEncoding() +
             "\"></span></td>\n" +
             "      <td>Procedure<span class=\"" + SemanticMarkup.PROCEDURE_MARK.getEncoding() + "\"></span></td>\n" +
             "    </tr>\n" +
             "    <tr>\n" +
             "      <td>Disease / Disorder<span class=\"" + SemanticMarkup.DISORDER_MARK.getEncoding() +
             "\"></span></td>\n" +
             "      <td>Medication<span class=\"" + SemanticMarkup.DRUG_MARK.getEncoding() + "\"></span></td>\n" +
             "    </tr>\n" +
             "    <tr>\n" +
             "      <td>Anatomical Site<span class=\"" + SemanticMarkup.ANATOMY_MARK.getEncoding() +
             "\"></span></td>\n" +
             "    </tr>\n" +
             "  </table>\n" +
             "  <hr>\n" +
             "  <table>\n" +
             "    <tr>\n" +
             "      <td>Coreference Element<span class=\"" + SemanticMarkup.ENTITY_MARK.getEncoding() +
             "\"><sup>1</sup></span></td>\n" +
             "    </tr>\n" +
             "  </table>\n" +
             "</div>\n";
