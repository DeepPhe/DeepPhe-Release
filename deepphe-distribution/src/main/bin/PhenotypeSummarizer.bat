::
::   Runs the Phenotype summarization pipeline.
::::
...
if exist "%DEEPPHE_HOME%\bin\PhenotypeSummarizer.bat" goto okHome
cd ..
set DEEPPHE_HOME=%cd%
...
java -cp "%CLASS_PATH%" %LOG4J_PARM% -Xms512M -Xmx3g %PIPE_RUNNER% -p %DEEPPHE_HOME%\data\pipeline\Phenotype.piper %*
cd %CURRENT_DIR%
